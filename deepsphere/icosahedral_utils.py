...
def _unique_rows(data, digits=None):
    """
    ...
    """
    hashes = _hashable_rows(data, digits=digits)
...
def _hashable_rows(data, digits=None):
    """
    ...
    as_int = _float_to_int(data, digits=digits)
...
def _float_to_int(data, digits=None, dtype=np.int32):
    """
    ...
    if digits is None:
        digits = _decimal_to_digits(1e-8)
        ...
        print('WARNING: Digits were passed as %s!', digits.__class__.__name__)
...
def _decimal_to_digits(decimal, min_digits=None):
    """
